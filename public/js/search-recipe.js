        ...
        const response = await fetch(`api/recipes/search/${recipeName}`, {
            ...
            .then(data => {
                for (let i = 0; i < data.length; i++) {
                    let card = `<article class="draggable container card p-0 row mx-2 border border-3 border-success" data-value=${data[i].recipe_name} draggable="true" id=${data[i].id} ondragstart="onDragStart(event);">
